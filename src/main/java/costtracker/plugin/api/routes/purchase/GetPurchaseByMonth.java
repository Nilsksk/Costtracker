...
public class GetPurchaseByMonth implements GetHandler {
    ...
    public void addGetHandler(HttpServer server, String path) {
        server.createContext(path, handler -> {
            try (handler) {
                ...
                int purchaseMonth = Integer.parseInt(bodyData.getString("month"));
                ...
                if (httpHeader.METHOD_GET.headerData.equals(requestMethod)) {
                    if (HandlerHelperFunctions.checkURI(handler.getRequestURI(), path)) {
                            ...
                            List<Purchase> purchaseList = purchaseHandler.getByMonth(purchaseMonth, purchaseYear);
